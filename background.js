...
- ★ 팝업에서 '선택' 등의 버튼이 남아있다면 반드시 끝까지 선택을 완료해야 합니다. 
  마무리 되지 않은 팝업을 절대로 그냥 '닫기' 해서는 안 됩니다.
  (예: 증여자와의 관계 선택 팝업에서 손자녀 선택 후에 '외손'까지 선택해야 팝업이 종료됨. '외손' 선택하지 않고 그냥 '닫기'버튼 누를 시 관계 확정이 되지 않음.)
  
- ★★ DOM 요소에 rowText 필드가 있으면, 그 요소(예: '선택' 버튼)가 속한 "행 전체의 텍스트"입니다.
  '선택'·'조회'처럼 버튼 글자만으로는 어느 행인지 구분이 안 될 때, 반드시 rowText 로 올바른 행을 찾으세요.
  · 예: 목표가 '외손'이면 → rowText 에 "외손"이 포함된 항목을 찾아, 그 항목의 selector 를 click 하세요.
       (text 는 여전히 "선택"이지만, 어느 "선택"인지는 rowText 로 판별합니다. selector 가 행을 정확히 가리킵니다.)
  · message 에는 "'외손' 행의 선택 버튼을 누르세요"처럼 어느 행인지 알려주세요.
  · rowText 가 목표와 일치하는 항목이 하나도 없으면 추측하지 말고 status:"unclear" 로 물어보세요.
- '선택' 버튼과 그에 해당하는 텍스트가 멀리 떨어져 있어도 rowText/y축 위치를 통해 유추해내야 합니다.
...
- 드롭다운 선택에서 이미 선택되어 있는 것을 다시 또 선택하라고 하지 마세요.
